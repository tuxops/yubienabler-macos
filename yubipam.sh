...
for myfile in ${AUTHFILE} ${SCREENSAVERFILE}; do
  if [[ ! -f ${PAMDIR}/${myfile} ]]; then
      echo "ERROR: ${THISONE} ${PAMDIR}/${myfile} doesn't exist, giving up" >&2
      exit 2
  fi
done
...
for myfile in ${AUTHFILE} ${SCREENSAVERFILE}; do
  ...
  if grep -v "^[[:blank:]]*#" ${PAMDIR}/${myfile}\
  | grep -q ${YUBILIB} 2>/dev/null
  then
    ...
    if sudo cp ${MYTMPDIR}/${myfile} ${PAMDIR}/${myfile} 2>/dev/null;then
      ...
      echo "ERROR: ${THISONE} couldn't amend ${PAMDIR}/${myfile}, giving up" >&2
      exit 3
    fi
  fi
done
